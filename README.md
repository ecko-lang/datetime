...
No native code, no capabilities. A datetime is just a `UTC` millisecond `Int`,
so two of them compare and subtract with plain `<` and `-`.
...
The calendar conversion is Howard Hinnant's `days`↔`civil` algorithm — pure
